...
public class SampleGui02 : GuiRenderer
{
    ...
    void CreateSkin()
    {
        ...
        tooltipStyle.Texture = "border";
        tooltipStyle.Tiling = TextureMode.Grid;
        tooltipStyle.Grid = new Margin(2);
        tooltipStyle.BackColor = ColorInt.ARGB(.9f, 0, 0, 0);
        ...
        inputStyle.Focused.Tint = ColorInt.ARGB(1, 1, 0, 0);
        ...
        borderStyle.Hot.Texture = "border";
        borderStyle.Pressed.Texture = "border";
        ...
        handleNW.Texture = "handleNW";

        ControlStyle handleNE = new ControlStyle();
        handleNE.Texture = "handleNE";
...
public class StatusBar : Control
{
    ...
    void button_OnDragDrop(Control sender, DragDropEventArgs e)
    {
    ...
    static Game()
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            string tooltip = "" +
                 ...
                 "[color=ff0066ff]" + "25% Chance to freeze on hit" + "[/color]" + Environment.NewLine +
                 "[color=ff0066ff]" + "11% Chance to cast Fireburst on attack" + "[/color]" + Environment.NewLine +
                 "[color=ff0066ff]" + "Cannot be repaired" + "[/color]";
...
public class Inventory : SampleWindow
{
    ...
    void Refresh()
    {
        ...
        foreach (Item item in Game.Player.Items)
        {
            ...
            button.MouseDrag += button_OnMouseDrag;

            flow.Controls.Add(button);
        }
    }

    void button_OnMouseDrag(Control sender, MouseEventArgs args)
    {
        ActionButton slot = sender as ActionButton;
...
public class Skillbook : SampleWindow
{
    ...
    void Refresh()
    {
        ...
        foreach (Skill item in Game.Player.Skills)
        {
            ActionButton button = new ActionButton(item);
            button.Size = new Point(40, 40);
            button.Style = "button";
...
public class CharacterSheet : SampleWindow
{
    ...
    public CharacterSheet()
    {
        ...
        ImageControl image = new ImageControl { Texture = "paperdoll" };
        image.Dock = DockStyle.Fill;
        ...
        int i = 0;
        foreach (KeyValuePair<ItemSlotType, ItemSlot> pair in Game.Player.ItemSlots)
        {
            ...
            button.DragDrop += button_OnDragDrop;
    ...
    void button_OnDragDrop(Control sender, DragDropEventArgs e)
    {
        ...
        if (e.DraggedControl.Tag is Item)
        {
            ...
            if (Game.Player.Equip(item, itemSlot))
                slot.Item = item;
        }
    }
}
